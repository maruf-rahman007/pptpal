...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ path, className, variant, size, asChild = false, ...props }, ref) => {
    const router = useRouter();
    const Comp = asChild ? Slot : "button";

    const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
      if (path && path !== "submit") {
        e.preventDefault(); // prevent form submit if navigating
        router.push(path);
      }

      // call user's onClick if exists
      if (props.onClick) {
        props.onClick(e);
      }
    };
    ...
    return (
      <Comp
        ...
        {...props}
        onClick={path && path !== "submit" ? handleClick : props.onClick}
      />
    );
  }
);
